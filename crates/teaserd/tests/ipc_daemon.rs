...
use teaserd::ipc::{MAX_REQUEST_BYTES, PROTOCOL_VERSION};
...
fn daemon_creates_a_session_over_versioned_ipc() {
	...
	let lock_permissions = permissions(&runtime.path().join("teaserd.lock"));
...
fn second_daemon_cannot_take_over_a_live_runtime() {
	...
	let mut second = spawn_teaserd(runtime.path());
...
impl TestDaemon {
	fn spawn(runtime: &TestRuntime) -> Self {
		...
		let child = spawn_teaserd(runtime.path());
...
fn spawn_teaserd(runtime_dir: &Path) -> Child {
