//! Versioned local control protocol for `teaserd`.
...
/// The protocol version understood by this build of `teaserd`.
...
const ATTACH_UPGRADE: &str = "teaser.attach.v1";

/// A Unix-domain-socket server for `teaserd` control and attachment requests.
///
...
impl IpcServer {
	...
	pub(crate) fn serve_next(&self, registry: &SessionRegistry) -> Result<(), IpcServerError> {
		...
		thread::Builder::new()
			.name("teaserd-ipc".to_owned())
			.spawn(move || {
				if let Err(error) = handle_connection(stream, &registry) {
					eprintln!("teaserd: connection failed: {error}");
				}
			})
...
fn serve_attachment(
	...
	let input_thread = thread::Builder::new()
		.name("teaserd-pty-input".to_owned())
		.spawn(move || {
