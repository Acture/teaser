//! Secure filesystem state required to start one `teaserd` process.
...
const LOCK_FILENAME: &str = "teaserd.lock";
const SOCKET_FILENAME: &str = "control.sock";

/// Exclusive ownership of one protected `teaserd` runtime directory.
#[derive(Debug)]
...
fn default_path_from_home(home: OsString) -> PathBuf {
	PathBuf::from(home).join("Library/Application Support/Teaser/runtime")
}
	...
	fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Self::AlreadyRunning { lock_path } => {
				write!(formatter, "another teaserd owns {}", lock_path.display())
			}
