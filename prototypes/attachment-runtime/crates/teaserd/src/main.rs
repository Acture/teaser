...
use teaserd::SessionRegistry;
use teaserd::runtime::DaemonRuntime;

mod probe;

const USAGE: &str = "Usage: teaserd [--runtime-dir PATH] [--probe-session]";

fn main() -> ExitCode {
	match run() {
		Ok(()) => ExitCode::SUCCESS,
		Err(message) => {
			eprintln!("teaserd: {message}");
			ExitCode::FAILURE
		}
	}
}
...
fn run() -> Result<(), String> {
	...
	eprintln!("teaserd: listening on {}", runtime.socket_path().display());
	loop {
		if let Err(error) = runtime.serve_next(&registry) {
			eprintln!("teaserd: control request failed: {error}");
		}
	}
}
...
mod tests {
	...
	fn parses_probe_session_with_runtime_directory_in_either_order() {
		let expected = Invocation::Run {
			runtime_dir: Some(PathBuf::from("/tmp/teaser-probe")),
			probe_session: true,
		};
		assert_eq!(
